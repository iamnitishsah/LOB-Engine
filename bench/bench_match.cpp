...
static void BM_Match_SingleLevel(benchmark::State& state) {
    ...
    for (auto _ : state) {
        engine.process_limit_order(1, maker_id++, lob::Side::Sell, 10050, 100);
        engine.process_limit_order(1, taker_id++, lob::Side::Buy, 10050, 100);
    }
...
static void BM_Match_FiveLevels(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (int p = 10050; p <= 10054; ++p) {
            engine.process_limit_order(1, maker_id++, lob::Side::Sell, p, 20);
        }
        engine.process_limit_order(1, taker_id++, lob::Side::Buy, 10054, 100);
    }
...
static void BM_MarketOrder_Sweep(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (int p = 10050; p <= 10059; ++p) {
            engine.process_limit_order(1, maker_id++, lob::Side::Sell, p, 10);
        }
        engine.process_market_order(1, taker_id++, lob::Side::Buy, 100);
    }
