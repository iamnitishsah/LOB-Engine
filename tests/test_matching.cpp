...
protected:
    void SetUp() override {
        ...
        engine = std::make_unique<lob::MatchingEngine>();
        engine->add_instrument(1, std::move(book));
        trades.clear();
...
TEST_F(MatchingEngineTest, FullMatchSingleLevel) {
    ...
    engine->process_limit_order(1, 1, lob::Side::Sell, 105, 100);
    EXPECT_EQ(engine->get_book(1)->order_count(), 1);
    EXPECT_EQ(trades.size(), 0);

    // Incoming bid: ID 2, Buy 100 @ 105
    engine->process_limit_order(1, 2, lob::Side::Buy, 105, 100);
    ...
    EXPECT_EQ(engine->get_book(1)->order_count(), 0);
    EXPECT_TRUE(engine->get_book(1)->verify_invariants());
}

TEST_F(MatchingEngineTest, PartialFillsAndRestingRemainder) {
    // Resting ask: ID 1, Sell 40 @ 105
    engine->process_limit_order(1, 1, lob::Side::Sell, 105, 40);
    ...
    EXPECT_EQ(engine->get_book(1)->order_count(), 1);
    EXPECT_EQ(engine->get_book(1)->get_best_bid(), 105);
    EXPECT_EQ(engine->get_book(1)->get_best_ask(), lob::INVALID_PRICE);

    const auto* bid_level = engine->get_book(1)->get_best_bid_level();
...
TEST_F(MatchingEngineTest, PriceTimePriority) {
    ...
    engine->process_limit_order(1, 1, lob::Side::Sell, 105, 50);
    engine->process_limit_order(1, 2, lob::Side::Sell, 105, 50);

    // Incoming bid matches 60: must fill ID 1 completely (50), then ID 2 partially (10)
    engine->process_limit_order(1, 3, lob::Side::Buy, 105, 60);
    ...
    EXPECT_EQ(engine->get_book(1)->order_count(), 1);
    const auto* ask_level = engine->get_book(1)->get_best_ask_level();
...
TEST_F(MatchingEngineTest, MarketOrderSweepMultipleLevels) {
    engine->process_limit_order(1, 1, lob::Side::Sell, 101, 10);
    engine->process_limit_order(1, 2, lob::Side::Sell, 102, 20);
    engine->process_limit_order(1, 3, lob::Side::Sell, 103, 30);

    // Market buy of 45: sweeps 101 (10), 102 (20), and 15 of 103
    lob::Qty filled = engine->process_market_order(1, 4, lob::Side::Buy, 45);
    ...
    EXPECT_EQ(engine->get_book(1)->get_best_ask(), 103);
    EXPECT_EQ(engine->get_book(1)->get_best_ask_level()->total_qty, 15);
    EXPECT_TRUE(engine->get_book(1)->verify_invariants());
}
